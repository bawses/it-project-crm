...
function swapContactInList(contactList: IContact[], contact: IContact) {
...
export default function Contacts() {
	...
	useEffect(() => {
		getSession().then((session) => {
			if (session) {
				if (session.user.type === DataType.User) {
					setIsLoading(false);
	...
	async function handleStarClick(target: IContact) {
		...
		try {
			...
			setErrorMessage(
				"Failed to update starred status - Please refresh the page and try again"
			);
	...
	let displayContactsComponent: JSX.Element = (
		...
		>
			{bigScreen ? (
				<Typography variant="h4">No contacts to display...</Typography>
			) : (
